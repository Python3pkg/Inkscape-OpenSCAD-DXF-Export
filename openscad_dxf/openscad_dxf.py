...
class OpenSCADDXFEffect(object2path.ObjectToPath):
    ...
    def dxf_add_dimension(self, name, x=None, y=None, layer=None):
        ...
        codes += [
            ...
            ('13', '%f' % (x[0] if x is not None and x[0] is not None else 0.0)),
            ('23', '%f' % (y[0] if y is not None and y[0] is not None else 0.0)),
            ('33', '0.0'),
            ('14', '%f' % (x[1] if x is not None and x[1] is not None else 0.0)),
            ('24', '%f' % (y[1] if y is not None and y[1] is not None else 0.0)),
            ('34', '0.0'),
